...
const createVet = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "vet") {
      return res
        .status(400)
        .json({ message: "Invalid user or user is not a vet" });
    }
    ...
    if (vet) {
      ...
      return res
        .status(200)
        .json({ message: "Vet profile updated successfully", vet });
    } else {
      ...
      return res
        .status(201)
        .json({ message: "Vet profile created successfully", vet });
    }
  } catch (error) {
    console.error(error);
...
const deleteVet = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Vet profile deleted successfully" });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
